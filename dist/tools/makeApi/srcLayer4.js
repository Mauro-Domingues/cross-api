...
export class MakeFourthLayer {
    ...
    async execute() {
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndCloneAttribute.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndCloneAttribute.ts'), this.createMapAndClone.execute());
        }
        else {
            truncateSync(resolve('src', 'utils', 'mappers', 'mapAndCloneAttribute.ts'));
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndCloneAttribute.ts'), this.createMapAndClone.execute());
        }
        console.log('\x1b[38;2;255;255;0m', `- mapAndCloneAttribute.ts.ts ${this.messages.created}`, '\x1b[0m');
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndInsertAttribute.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndInsertAttribute.ts'), this.createMapAndInsert.execute());
        }
        else {
            truncateSync(resolve('src', 'utils', 'mappers', 'mapAndInsertAttribute.ts'));
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndInsertAttribute.ts'), this.createMapAndInsert.execute());
        }
        console.log('\x1b[38;2;255;255;0m', `- mapAndInsertAttribute.ts ${this.messages.created}`, '\x1b[0m');
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndPatchAttribute.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndPatchAttribute.ts'), this.createMapAndPatch.execute());
        }
        else {
            truncateSync(resolve('src', 'utils', 'mappers', 'mapAndPatchAttribute.ts'));
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndPatchAttribute.ts'), this.createMapAndPatch.execute());
        }
        console.log('\x1b[38;2;255;255;0m', `- mapAndPatchAttribute.ts ${this.messages.created}`, '\x1b[0m');
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndPatchString.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndPatchString.ts'), this.createMapAndPatchString.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateAttribute.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateAttribute.ts'), this.createMapAndUpdate.execute());
        }
        else {
            truncateSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateAttribute.ts'));
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateAttribute.ts'), this.createMapAndUpdate.execute());
        }
        console.log('\x1b[38;2;255;255;0m', `- mapAndUpdateAttribute.ts ${this.messages.created}`, '\x1b[0m');
        if (!existsSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateString.ts'))) {
            appendFileSync(resolve('src', 'utils', 'mappers', 'mapAndUpdateString.ts'), this.createMapAndUpdateString.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'shared', 'errors', 'AppError.ts'))) {
            appendFileSync(resolve('src', 'shared', 'errors', 'AppError.ts'), this.createAppError.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'shared', 'typeorm', 'index.ts'))) {
            appendFileSync(resolve('src', 'shared', 'typeorm', 'index.ts'), this.createTypeorm.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'shared', 'typeorm', 'dataSource.ts'))) {
            appendFileSync(resolve('src', 'shared', 'typeorm', 'dataSource.ts'), this.createDataSource.execute());
        }
        else {
