...
export class MakeThirdLayer {
    ...
    async execute() {
        if (!existsSync(resolve('src', '@types', 'express.d.ts'))) {
            appendFileSync(resolve('src', '@types', 'express.d.ts'), this.createExpressNamespace.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', '@types', 'env.d.ts'))) {
            appendFileSync(resolve('src', '@types', 'env.d.ts'), this.createEnvNamespace.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'assets', 'domains.txt'))) {
            appendFileSync(resolve('src', 'assets', 'domains.txt'), this.createDomains.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'dtos', 'ICacheDTO.ts'))) {
            appendFileSync(resolve('src', 'dtos', 'ICacheDTO.ts'), this.createICacheDTO.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'dtos', 'IListDTO.ts'))) {
            appendFileSync(resolve('src', 'dtos', 'IListDTO.ts'), this.createIListDTO.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'dtos', 'IObjectDTO.ts'))) {
            appendFileSync(resolve('src', 'dtos', 'IObjectDTO.ts'), this.createIObjectDTO.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'dtos', 'IResponseDTO.ts'))) {
            appendFileSync(resolve('src', 'dtos', 'IResponseDTO.ts'), this.createIResponseDTO.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'middlewares', 'RateLimiter.ts'))) {
            appendFileSync(resolve('src', 'middlewares', 'RateLimiter.ts'), this.createRateLimiter.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'middlewares', 'EnsureAuthenticated.ts'))) {
            appendFileSync(resolve('src', 'middlewares', 'EnsureAuthenticated.ts'), this.createEnsureAuthenticated.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'middlewares', 'DecodeJwt.ts'))) {
            appendFileSync(resolve('src', 'middlewares', 'DecodeJwt.ts'), this.createDecodeJwt.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'routes', 'guardRouter.ts'))) {
            appendFileSync(resolve('src', 'routes', 'guardRouter.ts'), this.createGuard.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'shared', 'app.ts'))) {
            appendFileSync(resolve('src', 'shared', 'app.ts'), this.createApp.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'shared', 'server.ts'))) {
            appendFileSync(resolve('src', 'shared', 'server.ts'), this.createServer.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'utils', 'decimalAdjust.ts'))) {
            appendFileSync(resolve('src', 'utils', 'decimalAdjust.ts'), this.createDecimaAdjust.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'utils', 'domainsManager.ts'))) {
            appendFileSync(resolve('src', 'utils', 'domainsManager.ts'), this.createDomainsManager.execute());
        }
        else {
        ...
        if (!existsSync(resolve('src', 'utils', 'normalizeQueryLink.ts'))) {
            appendFileSync(resolve('src', 'utils', 'normalizeQueryLink.ts'), this.createNormalizeQueryLink.execute());
        }
        else {
