...
export class Console {
    ...
    one(assets) {
        const payload = [
            ...
            String(`${this.isbreakStart(assets[3])}${assets[0]}${this.isbreakend(assets[4])}`),
            String('\x1b[0m'),
        ];
    ...
    many(assets) {
        const payload = [];
        assets.map(asset => {
            return payload.push(String(this.isBold(asset[2])), String(this.getColor(asset[1])), String(`${this.isbreakStart(asset[3])}${asset[0]}${this.isbreakend(asset[4])}`), String('\x1b[0m'));
        });
        const output = payload.reduce((acc, input) => acc + input);
        return console.log(output);
    }
