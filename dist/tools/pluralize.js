...
class Pluralize {
  ...
  replaceWord(replaceMap, keepMap, rules) {
    if (!this.word) {
      console.log('\x1b[1m', '\x1b[38;2;255;0;0m', `this${this.messages.invalidLanguage}`, '\x1b[0m');
      throw new Error();
    }
  ...
  addPluralRule(rule, replacement) {
    if (typeof replacement === 'string') this.pluralRules.push([this.sanitizeRule(rule), replacement]);
  }
  addSingularRule(rule, replacement) {
    if (typeof replacement === 'string') this.singularRules.push([this.sanitizeRule(rule), replacement]);
  }
  ...
  isPlural() {
    return this.checkWord(this.irregularSingles, this.irregularPlurals, this.pluralRules);
  }
  isSingular() {
    return this.checkWord(this.irregularPlurals, this.irregularSingles, this.singularRules);
  }
  plural() {
    return this.replaceWord(this.irregularSingles, this.irregularPlurals, this.pluralRules);
  }
  singular() {
    return this.replaceWord(this.irregularPlurals, this.irregularSingles, this.singularRules);
  }
}
