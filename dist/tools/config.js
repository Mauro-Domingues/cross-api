...
function configJson() {
  ...
  const dependencies = ['aws-sdk', 'axios', 'bcrypt', 'celebrate', 'class-transformer', 'cors', 'csv-parse', 'date-fns', 'dotenv', 'express', 'express-async-errors', 'handlebars', 'ioredis', 'jsonwebtoken', 'mime', 'multer', 'mysql', 'nodemailer', 'rate-limiter-flexible', 'redis@^3.0.2', 'reflect-metadata', 'supertest', 'swagger-ui-express', 'ts-jest', 'tsyringe', 'typeorm', 'uuid', 'xlsx'];
  const devDependencies = ['@babel/cli', '@babel/core', '@babel/node', '@babel/plugin-proposal-class-properties', '@babel/plugin-proposal-decorators', '@babel/preset-env', '@babel/preset-typescript', '@types/bcrypt', '@types/cors', '@types/express', '@types/ioredis', '@types/jest', '@types/jsonwebtoken', '@types/mime', '@types/multer', '@types/nodemailer', '@types/redis@^2.8.27', '@types/shelljs', '@types/supertest', '@types/swagger-ui-express', '@types/uuid', '@typescript-eslint/eslint-plugin', '@typescript-eslint/parser', 'babel-plugin-module-resolver', 'babel-plugin-transform-typescript-metadata', 'eslint', 'eslint-config-airbnb-base', 'eslint-config-prettier', 'eslint-import-resolver-typescript', 'eslint-plugin-import', 'eslint-plugin-import-helpers', 'eslint-plugin-prettier', 'jest', 'prettier', 'ts-node-dev', 'tsconfig-paths', 'typescript'];
  console.log('');
  ...
  rl.question('Your answer: ', languageOption => {
    if (languageOption !== '0' && languageOption !== '1') {
      ...
      if (languageOption === '0') {
        ...
        console.log('');
        const devDependenciesToInstall = dependencies.reduce((acc, devDependency) => {
          acc += ` ${devDependency}`;
          return acc;
        });
        _shelljs.default.exec(`yarn add ${devDependenciesToInstall} -D`);
