...
exports.default = listSpecDependentService;
function listSpecDependentService(lowerModuleName, upperModuleName, pluralLowerModuleName, pluralUpperModuleName, pluralFatherLowerModuleName) {
  return `import Fake${upperModuleName}Repository from '@modules/${pluralFatherLowerModuleName}/repositories/fakes/Fake${pluralUpperModuleName}Repository';
...
describe('List${upperModuleName}Service', () => {
  ...
  it('should be able to list the ${pluralLowerModuleName} with the specified pagination', async () => {
    ...
    expect(${lowerModuleName}List02.data.list).toEqual([${lowerModuleName}01, ${lowerModuleName}02]);
  });
});
`;
}